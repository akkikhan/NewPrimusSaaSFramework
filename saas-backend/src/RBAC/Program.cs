...
Log.Logger = new LoggerConfiguration()
    ...
    .WriteTo.Console()
    .WriteTo.File("logs/rbac-.txt", rollingInterval: RollingInterval.Day)
    .CreateLogger();
...
app.MapGet("/health", () => Results.Ok(new { status = "healthy", service = "rbac", timestamp = DateTime.UtcNow }));

try
{
    Log.Information("Starting RBAC API");
    app.Run("http://localhost:5002");
}
catch (Exception ex)
{
    Log.Fatal(ex, "RBAC API terminated unexpectedly");
}
finally
{
    Log.CloseAndFlush();
